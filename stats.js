...
function grabStats( json, update ){
	//console.log( json );
	...
	if( update === true)
		...
		updatePlayersWinRates(json, champsPlayed);
}
...
**/
function updatePlayersWinRates(json, champsPlayed){
	var maxMatchID = json.matches['9'].matchId; // Maximum match ID given from API call
	var redundant = checkRedundantMatch( maxMatchID, window.localStorage.getItem('sID')); // Make sure that matches returned are not redundant
	if( redundant === "No Matches Updated"){
		for(var i=0; i < champsPlayed.length; i++){
			...
			console.log( winrates( champsPlayed[i][0], champsPlayed[i][1], win, curRank[0], matchID, window.localStorage.getItem('sID') ) );
		}
	}
	else{
		for(var i=0; i < champsPlayed.length; i++){
